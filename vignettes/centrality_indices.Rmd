...
#harmonic closeness
g %>% 
  indirect_relations(g,type="geodesic",FUN=dist_inv) %>% 
  aggregate_positions(type="sum")
```

The distance based indices constitute one of the biggest groups of indices in the `netrankr` package.

```{r distance indices,eval=F}
#residual closeness (Dangalchev,2006)
g %>% 
  indirect_relations(g,type="geodesic",FUN=dist_2pow) %>% 
  aggregate_positions(type="sum")

#generalized closeness (Agneessens et al.,2017) (alpha>0)
g %>% 
  indirect_relations(g,type="geodesic",FUN=dist_dpow,alpha=2) %>% 
  aggregate_positions(type="sum")

#residual closeness (Agneessens et al.,2017) (alpha>0)
g %>% 
  indirect_relations(g,type="geodesic",FUN=dist_dpow,alpha=2) %>% 
  aggregate_positions(type="sum")
...
g %>% 
  indirect_relations(g,type="geodesic",FUN=dist_powd,alpha=0.7) %>% 
  aggregate_positions(type="sum")
...
g %>% 
  indirect_relations(g,type="geodesic",FUN=dist_integration) %>% 
  aggregate_positions(type="sum")
