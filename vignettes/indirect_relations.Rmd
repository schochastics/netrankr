...
of the *observed* relations. As an example, consider (shortes path) distances in the 
...
#Logarithmic forest distance (parametrized relation)
LF <- indirect_relations(g, type = "dist_lf",log_param = 1) 
```
...
The functions `dist_` are thus only meaningful for `type="dist_sp"` or `type="dist_resist"`
