...
"Proximity" can be translated to the graph-theoretic concept of shortes path distances, 
...
exist various other indices, that are based on the shortes path distances in a graph.
...
We could know accept that there is no index based on shortes path distances that could 
...
the pairwise shortes path distances as our *indirect relation* of interest and calculate
...
(potentially) quite far from being able to explain wealth perfectly with shortes path 
